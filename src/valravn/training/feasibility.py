...
class FeasibilityRule:
    ...
    rule_id: str
    description: str
    check_fn: Callable[[list[str], list[str]], tuple[bool, str]]
...
class FeasibilityMemory:
    ...
    _DESTRUCTIVE_CMDS: frozenset[str] = frozenset(
        {
            ...
            "move",
            "cp",
            "copy",  # could overwrite evidence
            "dd",
            "tee",  # classic evidence-clobbering tools
        }
    )
    ...
    def _check_valid_command(self, cmd: list[str], evidence_refs: list[str]) -> tuple[bool, str]:
